...
DESCRIBE t_revenue_origin;

DESCRIBE t_revenue;

-- Tratanto o erro das Tabelas T_REVENUE_ORIGIN e T_REVENUE
ALTER TABLE t_revenue_origin
    ADD CONSTRAINT t_revenue_origin_pk PRIMARY KEY (id_revenue_origin);
...
-- Corrigindo o nome ds_expense_categor
ALTER TABLE t_expense_category
    RENAME COLUMN ds_expense_categor TO ds_expense_category;

-- Corrigindo a coluna nr_cpf (tipo de dado NUMBER -> VARCHAR2)
ALTER TABLE t_user ADD nr_cpf_temp VARCHAR2(14) NOT NULL;
ALTER TABLE t_user DROP COLUMN nr_cpf;
ALTER TABLE t_user
    RENAME COLUMN nr_cpf_temp TO nr_cpf;

COMMIT;

-- IMPLEMENTANDO A CRIAÇÃO DE IDs NO BANCO DE DADOS
CREATE SEQUENCE seq_id_user
    START WITH 1
    INCREMENT BY 1;

CREATE OR REPLACE TRIGGER trg_id_user
BEFORE INSERT ON t_user
FOR EACH ROW
WHEN (NEW.id_user IS NULL)
BEGIN
    SELECT seq_id_user.NEXTVAL INTO :NEW.id_user FROM dual;
END;
